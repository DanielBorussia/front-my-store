...
describe('<Login />', () => {
    ...
    test("login valid data", async () => {
        ...
        const inputE = screen.getByRole("textbox", { type: /email/i });
       ...
       // await user.type(screen.getByRole("textbox", { type: /password/i }), LoginFormMock.password);
        fireEvent.change(inputE, { target: { value: LoginFormMock.email }});
        console.log(prettyDOM(inputE));
        console.log("pass "+ inputPassword.value);
        console.log("email "+ inputE.value);
