...
describe('<MyOrderPreview />', () => {
    test('render content', () => {
        ...
        const buttonAddOrder = component.getByText('Realizar Compra');
        const circularProgress = component.getByTestId('loadingProgress');
        const image = component.getByAltText(state.cart[0].name);
        expect(buttonAddOrder).toBeInTheDocument();
        expect(image.src).toContain(state.cart[0].image);
        fireEvent.click(buttonAddOrder);
